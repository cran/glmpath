"step.length.cox" <- function(corrector, lambda2, x, d, rslist, wlist, min.lambda, max.arclength, frac.arclength, add.newvars, backshoot, approx.Gram, h0=NULL, eps=.Machine$double.eps)
  {
...
"corrector.cox" <- function(x, y, d, rslist, wlist, rept, method, active, tmpa, force.active, lambda, lambda2, b0, a0, bshoot.threshold, relax.lambda, trace, function.precision, eps = .Machine$double.eps)
  {
    nobs <- length(y)
    p <- length(tmpa)
    fk <- length(force.active)
    ...
    if (p > 0) {
      ...
      lenz <- 10 + (p+5)*nobs 
      zsmall <- rep(0, lenz)
      zsmall[1:7] <- c(nobs, lambda, lambda2, method, k, 0, function.precision)
...
"coxpath" <- function(data, nopenalty.subset = NULL, method = c("breslow", "efron"), lambda2 = 1e-5, max.steps = 10*min(n, m), max.norm = 100*m, min.lambda = (if (m >= n) 1e-3 else 0), max.vars = Inf, max.arclength = Inf, frac.arclength = 1, add.newvars = 1, bshoot.threshold = 0.1, relax.lambda = 1e-7, approx.Gram = FALSE, standardize = TRUE, function.precision = 3e-13, eps = .Machine$double.eps, trace = FALSE)
  {
    ...
    corrector <- corrector.cox(x, time, status, rslist, wlist, rept, mthd, nopenalty.subset, nopenalty.subset, force.active, 0, lambda2, b, NULL, bshoot.threshold, relax.lambda, trace, function.precision)
    ...
    while(TRUE) {
      if (!backshoot) {
        ...
        k <- k + 1
        if (trace) cat(paste("\nStep",k,":"))
        step <- step.length.cox(corrector, lambda2, x, status, rslist, wlist, min.lambda, arclength, frac.arclength1, add.newvars, backshoot, approx.Gram)
        ...
        if (trace) cat(paste("\nStep",k,":"))
        step <- step.length.cox(corrector, lambda2, x, status, rslist, wlist, min.lambda, Inf, 1, add.newvars, backshoot, approx.Gram, h)
        step.len[k-1] <- h + step$h
        h <- step$h
        lam.vec[k] <- lambda <- lambda + h
        if (is.null(force.active)) a <- abs(b[tmpa])
      ...
      corrector <- corrector.cox(x, time, status, rslist, wlist, rept, mthd, active, tmpa, force.active, lambda, lambda2, b, a, bshoot.threshold, relax.lambda, trace, function.precision)
      newa <- corrector$newa
      while(length(newa) > 0) {
        if (trace) cat(paste("\nRepeating step",k,":"))
        tmpa <- c(tmpa, newa)
    ...
    aic <- -2*lp + 2*df
    bic <- -2*lp + log(n)*df
    object <- list(call = call, lambda=lam.vec[1:k], lambda2=lambda2, step.length=abs(step.len[1:(k-1)]), corr = cmat, new.df = new.df[1:k], df = df, loglik = lp, aic = aic, bic = bic, b.predictor = bmat.pred, b.corrector = bmat.corr, new.A = new.A[1:k], actions=actions[1:k], meanx = meanx, sdx = sdx, xnames = xnames, method = method, nopenalty.subset = nopenalty.subset, standardize = standardize)
    class(object) <- "coxpath"
    object
  }

"plot.coxpath" <- function(x, xvar = c("norm", "lambda", "step"), type = c("coefficients", "aic", "bic"), plot.all.steps = FALSE, xlimit = NULL, predictor = FALSE, omit.zero = TRUE, breaks = TRUE, mar = NULL, main = NULL, eps = .Machine$double.eps, ...)
  {
    object <- x
    ii <- object$new.A
    if (plot.all.steps) {
...
"summary.coxpath" <- function(object, ...)
  {
    ...
    ii <- object$new.A
    ii[length(ii)] <- TRUE
    M <- data.frame(Df = object$df[ii], Log.p.lik = object$loglik[ii], AIC = object$aic[ii], BIC = object$bic[ii])
