...
"plot.glmpath" <- function(x, xvar = c("norm", "lambda", "step"), type = c("coefficients", "aic", "bic"), plot.all.steps = FALSE, xlimit = NULL, predictor = FALSE, omit.zero = TRUE, breaks = TRUE, mar = NULL, eps = .Machine$double.eps, main = NULL, ...)
  {
    object <- x
    ii <- object$new.A
    if (plot.all.steps) {
...
"cv.glmpath" <- function(x, y, data, family = binomial, weight = rep(1, n), offset = rep(0, n), nfold = 10, fraction = seq(from=0, to=1, length=100), type = c("loglik","response"), mode = c("norm","lambda"), plot.it = TRUE, se = TRUE, ...)
  {
    type <- match.arg(type)
    mode <- match.arg(mode)
    if (is.character(family)) 
    ...
    for (i in seq(nfold)) {
        omit <- folds[[i]]
        fit <- glmpath(x[-omit, ], y[-omit], family=family, weight=weight[-omit], offset=offset[-omit], ...)
        pred <- switch(mode, norm = {
          predict(fit, x[omit, , drop = FALSE], y[omit], offset[omit], s = fraction,
...
"summary.glmpath" <- function(object, ...)
  {
    ...
    ii <- object$new.A
    ii[length(ii)] <- TRUE
    M <- data.frame(Df = object$df[ii], Deviance = object$deviance[ii], AIC = object$aic[ii], BIC = object$bic[ii])
    dimnames(M)[[1]] <- paste("Step", which(ii), sep=" ")
    M
  }

"bootstrap.path" <- function(x, y, data, B, index=NULL, path=c("glmpath","coxpath"), method=c("aic","bic"), trace=FALSE, ...)
  {
    path <- match.arg(path)
...
"plot.bootpath" <- function(x, type=c("histogram","pairplot"), mfrow = NULL, mar = NULL, ...)
  {
    ...
    if (type=="histogram") {
      ...
      for (j in 1:p) {
        hist(x[ ,j], main=dimnames(x)[[2]][j], xlab="Bootstrap coefficient", ylab="Frequency", freq=T, ...)
